...
export class PaymobController {
  ...
  async processPayment(
    ...
    try {
      ...
      const sameLevelOrders = userCompletedOrders.filter(
      ...
      const locale = paymentIntentionDto.locale || 'en';

      const data = {
        amount: finalPrice,
        currency: 'SAR', // <- Saudi Riyal
        payment_methods: [integration_id, 24018], // added 24018 for Tamara
        redirection_url: `${frontendUrl}/${locale}/app/levels/${paymentIntentionDto.level_name}`, // Redirect to level dashboard so student can see the structure
        items: [
          {
